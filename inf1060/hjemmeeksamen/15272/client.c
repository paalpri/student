 ...
 * which is 257 because of the biggest length can be is 255 ( unsigned char max)
 * and +2 for jobtype and length
 */
#define MAX_BUF 257

/* Here i have all the variables that i use in multiple functions,
 * instead of sending them to every function
 */
int sock;
int pipefd[2][2];
pid_t pid[2] = { -1, -1 };
...
int host_to_ip(char* hostname, char  *ip, char *port){
    ...
    /* here i should check every address in the linked list, and if one fails, try to connect to the next one
     * but since i was a bit short on time i dident prioritised this,
     * and every scenario my program is used in there is mostly only 1 ip address anyway.
     * I made the loop to show how to get all the addressen in the linked list,
     * and inside this loop i would test the socket() and bind(),
     * where if they failed i would try to connect to the next one.  
     */
...
int main(int argc, char *argv[]){
    ...
    char ip[100];
    if(host_to_ip(argv[1],ip,argv[2])){
